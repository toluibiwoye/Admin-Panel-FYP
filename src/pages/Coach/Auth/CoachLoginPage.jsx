...
const CoachLoginPage = () => {
	...
	const onSubmit = async (data) => {
		try {
			...
			const result = await sdk.login(data.email, data.password, "coach");
			if (!result.error) {
				dispatch({
					type: "LOGIN",
					payload: result,
				...
				navigate(redirect_uri ?? "/coach/dashboard");
			} else {
			...
			<div className="flex min-h-full flex-col items-center justify-center">
				<div className="my-12 flex w-[50%] flex-col items-center rounded-lg border border-[#a8a8a8] p-4  shadow-md">
					...
					<div className="flex items-center text-sm">
						...
						<Link to="/coach/sign-up" className="text-[#4F46E5]">
							Sign up here
						</Link>
					</div>
